...
class Sky:
    def __init__(self, horizon, style='level'):
        ...
        if self.style == 'overworld':
            palm_sufaces = import_folder('../graphics/overworld/palms')
            self.palms = []

            for surface in [choice(palm_sufaces) for image in range(10)]:
                x = randint(0, screen_width)
                y = (self.horizon * tile_size) + randint(50, 100)
                rect = surface.get_rect(midbottom=(x, y))
                self.palms.append((surface, rect))

            cloud_sufaces = import_folder('../graphics/overworld/clouds')
            self.clouds = []

            for surface in [choice(cloud_sufaces) for image in range(10)]:
                x = randint(0, screen_width)
